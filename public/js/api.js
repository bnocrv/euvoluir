...
export async function loadCourseManifest(courseId) {
  ...
  if (!res.ok) throw new Error(`Nao consegui carregar manifest do curso: ${courseId}`);
  return res.json();
}
...
export async function loadLesson(courseId, lessonId) {
  ...
  if (!item) throw new Error(`Aula nao encontrada no manifest: ${lessonId}`);

  const dataDir = resolveCourseDataDir(courseId);
  const res = await fetch(`./data/${dataDir}/${item.file}`);
  if (!res.ok) throw new Error(`Nao consegui carregar a aula: ${item.file}`);
  return res.json();
}
